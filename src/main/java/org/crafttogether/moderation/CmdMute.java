        ...
        if(staff){
            ...
            if(StringUtils.isNumeric(args[1].substring(0, args[1].length()-1))){
                ...
                if(commandSender instanceof Player){
                    new Punishment(PunishmentType.MUTE, ((Player)commandSender).getUniqueId(), UUID.randomUUID(), duration, false, LocalDateTime.now());
                } else {
                    new Punishment(PunishmentType.MUTE, null, UUID.randomUUID(), duration, false, LocalDateTime.now());
                }
                return true;
            }
            commandSender.sendMessage(USAGE);
